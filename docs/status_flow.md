...
- 바리오 보조 전환(리프트):
  - GPS 업데이트 지연 **≥ 10초**
  - 바리오 판정 주기 **1초**
  - 연속 판정 **3틱(≈3초 연속 조건 만족)**
  - Resting→OnLift: 최근 10초 누적 상승 **> 5m**
  - Riding→OnLift: 최근 10초 누적 상승 **> 7m**
...
## GPS 업데이트 지연 시 바리오 보조 전환
- 전제: `isTracking == true` + `isUsingBarometer == true`
- GPS 업데이트가 **10초 이상 지연**되면 바리오만으로 리프트 전환을 보조 평가
- **1초 주기 타이머**로 최근 10초 누적 상승량을 확인
- **3틱 연속 충족** 시 `onLift`로 전환
  - Resting: `recentGain > 5m`
  - Riding: `recentGain > 7m`
- 목적: GPS가 끊긴 구간에서도 **리프트 탑승을 놓치지 않기 위한 안전장치**
- 오늘 이슈(첫 런 후 리프트 탑승 시 `riding` 유지) 케이스는 **GPS 업데이트 공백으로 상태 판정이 실행되지 않은 것**이 핵심 원인으로 추정됨.
  - GPX 기준 18:33 KST(09:33Z) 근처 **100초+ 공백**이 존재했고, 그 구간 전후 상태가 `riding`으로 유지됨.
  - 이 보조 로직은 **GPS가 지연될 때도 바리오 상승 신호로 `onLift` 전환**을 가능하게 하여 같은 오염을 줄이는 목적임.
